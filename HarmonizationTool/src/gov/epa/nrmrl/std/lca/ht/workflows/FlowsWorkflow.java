...
public class FlowsWorkflow extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		lblActions.setLayoutData(new GridData(SWT.CENTER, SWT.CENTER, false,
				false, 1, 1));
		...
		GridData gd_label_01 = new GridData(SWT.RIGHT, SWT.CENTER, false,
				false, 1, 1);
		...
		GridData gd_btnLoadCSV = new GridData(SWT.CENTER, SWT.CENTER, false,
				false, 1, 1);
		...
		textLoadCSV.setBackground(SWTResourceManager
		...
		GridData gd_label_02 = new GridData(SWT.RIGHT, SWT.CENTER, false,
				false, 1, 1);
		...
		GridData gd_btnCheckData = new GridData(SWT.CENTER, SWT.CENTER, false,
				false, 1, 1);
		...
		GridData gd_textIssues = new GridData(SWT.FILL, SWT.CENTER, true,
				false, 1, 1);
		...
		GridData gd_label_03 = new GridData(SWT.RIGHT, SWT.CENTER, false,
				false, 1, 1);
		...
		GridData gd_btnCSV2TDB = new GridData(SWT.CENTER, SWT.CENTER, false,
				false, 1, 1);
		...
		textCommit.setBackground(SWTResourceManager
				.getColor(SWT.COLOR_INFO_BACKGROUND));
		textCommit.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false, 1, 1));

		// ======== ROW 4 =======================

		label_04 = new Label(composite, SWT.NONE);
		GridData gd_label_04 = new GridData(SWT.RIGHT, SWT.CENTER, false,
				false, 1, 1);
		...
		GridData gd_btnMatchFlowContexts = new GridData(SWT.CENTER, SWT.CENTER,
				false, false, 1, 1);
		...
		textMatchFlowContexts.setBackground(SWTResourceManager
				.getColor(SWT.COLOR_INFO_BACKGROUND));
		// textMatchFlowContexts.setText("(0 of 30");
		GridData gd_textMatchFlowContexts = new GridData(SWT.FILL, SWT.CENTER,
				true, false, 1, 1);
		...
		GridData gd_btnMatchFlowProperties = new GridData(SWT.CENTER,
				SWT.CENTER, false, false, 1, 1);
		btnMatchFlowProperties
				.addSelectionListener(matchFlowPropertiesListener);
		...
		textMatchFlowProperties = new Text(composite, SWT.BORDER
				| SWT.READ_ONLY);
		textMatchFlowProperties.setBackground(SWTResourceManager
				.getColor(SWT.COLOR_INFO_BACKGROUND));
		// textMatchFlowProperties.setText("(0 of 30");
		GridData gd_textMatchFlowProperties = new GridData(SWT.FILL,
		...
		GridData gd_label_06 = new GridData(SWT.RIGHT, SWT.CENTER, false,
				false, 1, 1);
		...
		GridData gd_btnMatchFlowables = new GridData(SWT.CENTER, SWT.CENTER,
				false, false, 1, 1);
		...
		textMatchFlowables.setBackground(SWTResourceManager
				.getColor(SWT.COLOR_INFO_BACKGROUND));
		// textMatchFlowables.setText("(430 of 600 rows match)");
		GridData gd_textMatchFlowables = new GridData(SWT.FILL, SWT.CENTER,
				true, false, 1, 1);
		...
		GridData gd_label_07 = new GridData(SWT.RIGHT, SWT.CENTER, false,
				false, 1, 1);
		...
		GridData gd_btnConcludeFile = new GridData(SWT.CENTER, SWT.CENTER,
				false, false, 1, 1);
		...
		private void doit(SelectionEvent e) {
			...
			try {
				handlerService.executeCommand(
						"gov.epa.nrmrl.std.lca.ht.handler.ImportCSV", null);
			} catch (Exception ex) {
				ex.printStackTrace();
			}
	...
	SelectionListener checkDataListener = new SelectionListener() {
		private void doit(SelectionEvent e) {
			...
			LCADataPropertyProvider[] lcaDataPropreties = TableKeeper
					.getTableProvider(CSVTableView.getTableProviderKey())
			...
			for (LCADataPropertyProvider lcaDataPropertyProvider : lcaDataPropreties) {
				...
				if (lcaDataPropertyProvider.isRequired()) {
					requiredLCADataPropertyProvider
							.add(lcaDataPropertyProvider);
				}
			}
			// FIXME LINES BELOW COMMENTED OUT TO MAKE CODE COMPILE

			if (checkForRequiredFlowableFields) {
				for (LCADataPropertyProvider requiredLCADataProperty : Flowable
						.getDataPropertyMap().values()) {
					if (requiredLCADataProperty.isRequired()) {
						...
						if (found == false) {
							new GenericMessageBox(e.display.getActiveShell(),
									"Missing Assignment",
									"For each flowable, the "
											+ requiredLCADataProperty
													.getPropertyName()
											+ " is required");
						}
					}
			...
			if (checkForRequiredFlowContextFields) {
				for (LCADataPropertyProvider requiredLCADataProperty : FlowContext
						.getDataPropertyMap().values()) {
					if (requiredLCADataProperty.isRequired()) {
						boolean found = false;
						for (LCADataPropertyProvider gotIt : requiredLCADataPropertyProvider) {
			...
			if (checkForRequiredFlowPropertyFields) {
				for (LCADataPropertyProvider requiredLCADataProperty : FlowProperty
						.getDataPropertyMap().values()) {
					if (requiredLCADataProperty.isRequired()) {
						boolean found = false;
						for (LCADataPropertyProvider gotIt : requiredLCADataPropertyProvider) {
			...
			if (countOfAssignedFields == 0) {
				...
				textCheckData.setText(issueCount + " issues. "
						+ countOfAssignedFields + " columns checked");
				if (issueCount == 0) {
	...
	SelectionListener commitListener = new SelectionListener() {
		...
		private void doit(SelectionEvent e) {
			...
			autoMatchJob.addJobChangeListener(new AutoMatchJobChangeListener(
					(FlowsWorkflow) Util.findView(FlowsWorkflow.ID), jobKey));
			...
			CSVTableView
					.setUniqueFlowContextRowNumbers(uniqueFlowContextRowNumbers);
			CSVTableView
					.setUniqueFlowPropertyRowNumbers(uniqueFlowPropertyRowNumbers);
			CSVTableView
					.setMatchedFlowableRowNumbers(matchedFlowableRowNumbers);
			CSVTableView
					.setMatchedFlowContextRowNumbers(matchedFlowContextRowNumbers);
			CSVTableView
					.setMatchedFlowPropertyRowNumbers(matchedFlowPropertyRowNumbers);
		}
...
//				MatchProperties.initialize();
	...
	public static void addContextRowNum(int rowNumToSend) {
		...
		textMatchFlowContexts
				.setText(matchedFlowContextRowNumbers.size() + " matched. "
	...
	public static void addPropertyRowNum(int rowNumToSend) {
		...
		textMatchFlowProperties.setText(matchedFlowPropertyRowNumbers.size()
				+ " matched. " + uniqueFlowPropertyRowNumbers.size()
				+ " found.");
		CSVTableView.colorFlowPropertyRows();
	}
	...
	public static void addFlowableRowNum(int rowNumToSend) {
		...
		textMatchFlowables.setText(matchedFlowableRowNumbers.size()
				+ " matched. " + uniqueFlowableRowNumbers.size() + " found.");
		CSVTableView.colorFlowableRows();
		// int count =
		// TableKeeper.getTableProvider(CSVTableView.getTableProviderKey()).getData().get(rowNumToSend)
